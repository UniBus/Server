...
function writeRandomTestHTMLStop($city, $items)
{
	...
	for ($i=1; $i<=5; $i++) {
		...
		echo '<li><a href=http://unibus.ca:5144/ver1.3/'.$city.'/arrivals.php?stop_id='.$randomStop['stop_id'].'>'.$randomStop['stop_name'].'</a>'.
		        ' [<a href=http://unibus.ca:5144/ver1.3/'.$city.'/stopmap.php?lat='.$randomStop['stop_lat'].'&long='.$randomStop['stop_lon'].'>map</a>]'.
		     '</li>';
	}
	echo '</ul>';
...
function writeRandomTestHTMLSchedule($city, $items)
{
	...
	for ($i=1; $i<=5; $i++) {
		...
		echo '<li><a href=http://unibus.ca:5144/ver1.3/'.$city.'/schedules.php?stop_id='.$randomTrip['stop_id'].
...
function writeRandomTestHTMLRoute($city, $items)
{
	...
	for ($i=1; $i<=5; $i++) {
		...
		echo '<li><a href=http://unibus.ca:5144/ver1.3/'.$city.'/tripsofroute.php?route_id='.$randomRoute['route_id'].'>'.$randomRoute['route_short_name'].' (' . $randomRoute['route_long_name']. ')'.'</a></li>';
	}
	echo '</ul>';
	closeDb($link);
}
...
function writeRandomTestHTMLTrip($city, $items)
{
	...
	for ($i=1; $i<=5; $i++) {
		$randomTrip = getRandomTrip($link);
		echo '<li><a href=http://unibus.ca:5144/ver1.3/'.$city.'/routemap.php?trip_id='.$randomTrip['trip_id'].'>'.$randomTrip['route_id'].' (' . $randomRoute['trip_headsign']. ')'.'</a></li>';
	}
	echo '</ul>';
	closeDb($link);
}
